...
const scanHolders = async (tick = 'ncat') => {

    if(global.tokens.indexOf(tick) <= -1)
        global.tokens.push(tick)
        ...
        for(let j = 0; j < data.length; j ++) {
            ...
            if(rec.tick === tick) {
                if (rec.amt && parseInt(rec.amt) > 0)
                    balance_list.push ( {_address: address, _amount: rec.amt})
            }
        }
    }
    ...
    balance_list.sort((n1, n2) => n2._amount - n1._amount)
